                    ...
                    <div className="flex-1 overflow-y-auto space-y-6 pr-4">
                        {messages.map(msg => (
                            <div key={msg.id}
                                ...
                                {msg.sender === 'bot' && <div
                                    className="w-8 h-8 flex-shrink-0 bg-blue-100 rounded-full text-lg flex items-center justify-center">🤖</div>}
                                <div
                                    className={`max-w-xl p-4 rounded-2xl ${msg.sender === 'bot' ? 'bg-gray-100 text-gray-800' : 'bg-blue-600 text-white'}`}>
                                    {msg.isLoading ? (
                                        <div className="flex items-center gap-2">
                                            <span className="w-2 h-2 bg-gray-400 rounded-full animate-pulse"></span>
                                            <span
                                                className="w-2 h-2 bg-gray-400 rounded-full animate-pulse delay-150"></span>
                                            <span
                                                className="w-2 h-2 bg-gray-400 rounded-full animate-pulse delay-300"></span>
                                        </div>
                                    ) : (
