...
const Icon = ({ path, className = "w-5 h-5" }: { path: string, className?: string }) => (
    <svg className={className} fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d={path}></path></svg>
);
...
const ALL_CODE_BLOCKS: CodeBlock[] = [
    ...
    { id: 'block-3', title: '비밀번호 암호화', content: "const hashedPassword = await bcrypt.hash(password, 10);", category: '보안' },
    { id: 'block-4', title: '새 사용자 생성', content: "const newUser = await db.user.create(...);", category: 'DB' },
    { id: 'block-5', title: 'JWT 토큰 생성', content: "const token = jwt.sign({ userId: newUser.id }, 'secret');", category: '보안' },
...
function SortableCodeBlock({ id, title, content, className, onRemove }: { id: string, title: string, content: string, className: string, onRemove?: (id: string) => void }) {
    const {
        attributes,
        listeners,
        setNodeRef,
    ...
    return (
        <div ref={setNodeRef} style={style} {...attributes} {...listeners} className={`p-3 rounded-md shadow cursor-grab text-sm transition-colors ${className}`}>
            <div className="flex items-start justify-between w-full">
                <div className="flex-grow">
                    <div className="text-xs text-gray-300 mb-1 flex items-center">
                        <svg className="w-3 h-3 mr-1.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"></path></svg>
                        {title}
                    </div>
                    <div className="font-mono text-white pl-1">{content}</div>
                </div>
                {onRemove && (
                    <button onClick={() => onRemove(id)} className="text-gray-400 hover:text-white font-bold text-xl p-1 -mt-2 -mr-1">
                        ×
                    </button>
                )}
            </div>
        ...
        <div className="flex flex-col items-center justify-center text-center py-20">
            ...
            <div className="flex gap-6">
                <Link href="/job-recommendations">
                    <button className="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-3 px-8 text-lg rounded-full shadow-lg transition-transform hover:scale-105">
                        다른 캐릭터 체험하기
                    </button>
                </Link>
                {/* '체험 끝내기' 버튼 클릭 시 onFinishExperience 함수를 호출하도록 수정 */}
                <button onClick={onFinishExperience} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 text-lg rounded-full shadow-lg transition-transform hover:scale-105">
                    체험 끝내기
                </button>
            </div>
        </div>
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4 mb-10">
                    {EXPERIENCED_JOBS.map((job) => (
                        ...
                        >
                            ...
                            <div className={`font-bold text-sm ${selectedJobId === job.id ? 'text-white' : 'text-purple-100'}`}>{job.name}</div>
                            <div className={`text-xs ${selectedJobId === job.id ? 'text-blue-100' : 'text-purple-200'}`}>{job.title}</div>
                        </div>
                    ))}
                </div>
...
export default function BackendQuestPage() {
    ...
    const questStages = [
        ...
        { name: "API 개발", status: "진행중", icon: "M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.5L15.232 5.232z" },
                            ...
                            <>
                                ...
                                <header className="flex justify-between items-center mb-6">
                                    <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 flex items-center gap-2">
                                        직무 체험 퀘스트 <span className="text-sm font-medium text-purple-700 bg-purple-100 px-3 py-1 rounded-full">백엔드 개발자</span>
                                    </h1>
                                ...
                                {/* ...기존 퀘스트 UI... */}
                                {/* 소개 및 조언 박스 */}
                                <div className="bg-green-50 border-2 border-green-200 rounded-2xl p-6 mb-8 text-green-800">
                                    ...
                                    <div className="bg-green-100 rounded-lg p-3 text-sm">
                                        <span className="font-bold">💡 곰의 조언:</span> 백엔드는 눈에 안 보이지만 모든 걸 떠받치는 기둥이야. 안정성이 최우선!
                                    </div>
                                </div>
                                {/* 퀘스트 진행 단계 */}
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                                    {questStages.map(stage => (
                                        <div key={stage.name} className={`p-4 rounded-xl border-2 ${stage.status === '진행중' ? 'bg-yellow-50 border-yellow-400' : 'bg-white'}`}>
                                            <div className="flex items-center gap-3">
                                                <div className={`p-2 rounded-full ${stage.status === '진행중' ? 'bg-yellow-100 text-yellow-600' : 'bg-gray-100 text-gray-500'}`}>
                                ...
                                <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                                    <div className="bg-white rounded-2xl shadow-lg p-6">
                                        <div className="flex flex-col lg:flex-row gap-5">
                                            {/* 왼쪽: 사용 가능 블록 목록 */}
                                            <aside id="available" className="w-full lg:w-1/4 bg-gray-800 text-white rounded-lg p-4 space-y-4">
                                                <SortableContext items={blockLists.available.map(b => b.id)} strategy={verticalListSortingStrategy}>
                                                    {Object.entries(categorizedAvailableBlocks).map(([category, blocks]) => (
                                                        <div key={category}>
                                                            ...
                                                            <div className="space-y-2">
                                                                {blocks.map(block => {
                                                                    const colors = { API: 'bg-blue-600 hover:bg-blue-700', 보안: 'bg-purple-600 hover:bg-purple-700', DB: 'bg-green-600 hover:bg-green-700' };
                                                                    return <SortableCodeBlock key={block.id} id={block.id} title={block.title} content={block.content} className={`${colors[block.category]}`} />;
                                                                })}
                                                            </div>
                                                        </div>
                                                    ))}
                                            ...
                                            <div className="w-full lg:w-1/2 flex flex-col">
                                                <main className="bg-gray-900 rounded-xl shadow-inner flex-col flex-grow">
                                                    <div className="flex-shrink-0 p-3 text-sm font-semibold text-gray-300 border-b border-gray-700">회원가입 API</div>
                                                    <div id="solution" className="p-4 space-y-2 flex-grow min-h-[350px]">
                                                        <SortableContext items={blockLists.solution.map(b => b.id)} strategy={verticalListSortingStrategy}>
                                                            {blockLists.solution.map(block => (
                                                                <SortableCodeBlock key={block.id} id={block.id} title={block.title} content={block.content} className="bg-gray-700" onRemove={handleRemoveBlock} />
                                                            ))}
                                                        </SortableContext>
                                                ...
                                                <div className="flex justify-center items-center gap-4 pt-4">
                                                    <button onClick={handleCodeExecution} className="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-6 rounded-lg text-sm">코드 실행</button>
                                                    <button onClick={handleReset} className="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-6 rounded-lg text-sm">초기화</button>
                                                </div>
                                            </div>

                                            {/* 오른쪽: 실행 결과 및 진행상황 */}
                                            <aside className="w-full lg:w-1/4">
                                                <div className="bg-gray-900 rounded-xl text-white text-xs p-4 font-mono h-[250px] overflow-y-auto">
                                                    <h3 className="font-bold text-gray-400 text-sm mb-2">실행 결과</h3>
                                                    {executionResult.map((line, index) => (
                                                        <p key={index} className={`${line.includes('🎉') || line.includes('✓') ? 'text-green-400' : ''} ${line.includes('❌') || line.includes('💡') ? 'text-yellow-400' : ''}`}>{line}</p>
                                                    ))}
                                                </div>
                                                <div className="mt-4 bg-white rounded-lg p-4 border">
                                                    ...
                                                    <ul className="space-y-2 text-sm text-gray-600">
                                                        {progressSteps.map(step => (
                                                            <li key={step.name} className={`flex items-center ${step.done ? 'text-gray-800 font-semibold' : 'text-gray-500'}`}>
                                                                <span className={`w-4 h-4 mr-2 rounded-full flex items-center justify-center ${step.done ? 'bg-green-500 text-white' : 'bg-gray-300'}`}>
                                                                    {step.done && '✓'}
                                                                </span>
                                                                {step.name}
                                                            </li>
                                ...
                                {/* 하단 완료 버튼 */}
                                <div className={`flex justify-end mt-6 transition-opacity duration-500 ${isQuestComplete ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                                    {isQuestComplete && (
                                        <button onClick={() => setIsQuestFinished(true)} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 text-lg rounded-full shadow-lg transition-transform hover:scale-105">
                                            퀘스트 완료하기
                                        </button>
                                    )}
                                </div>
